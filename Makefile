...
CILLY_FEATURES += lidomain

    # Now rules to make cilly
CILLY_LIBRARY_MODULES = linda pretty inthash errormsg alpha trace stats util clist \
	      ...
	      dataslicing sfi expcompare\
              ...
              testcil  rankEquation lidoutil heapify frontcUtil frontcCfg frontcTrans frontcProfiler\
              mergeByteOp union2struct lidoarray lidostruct formSIMDArray globalVariableToPointer\
              vortex\
	      $(CILLY_FEATURES) \
...
CILLY_LIBS     = unix str nums bigarray

SOURCEDIRS  += src src/frontc src/ext src/ext/pta ocamlutil src/ext/lido
...
odoc: $(OBJDIR)/pretty.cmi $(OBJDIR)/cil.cmi
	...
	ocamldoc -d doc/html/cil/api -v -stars \
             -html \
             -t "CIL API Documentation (version 1.3.6)" \
...
# Create the version document
	cd doc/html/cil; echo "\def\cilversion{1.3.6}" >cil.version.tex
...
pdfdoc: doc/cilpp.tex $(OBJDIR)/pretty.cmi $(OBJDIR)/cil.cmi
	cd doc; echo "\def\cilversion{1.3.6}" >cil.version.tex
...
DISTRIB_SRC_FRONTC = cabs.ml cabshelper.ml cprint.ml clexer.mli clexer.mll \
		     ...
		     cabs2cil.ml cabs2cil.mli frontc.ml frontc.mli \
	             cabsvisit.mli cabsvisit.ml patch.mli patch.ml frontcTrans.ml
...
BINCIL_TAR_GZ:=cil-win32-1.3.6.tar.gz
